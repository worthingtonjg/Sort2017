...
public class CM_RuntimeSetupExample extends MonoBehaviour {

	public var audioClip : AudioClip; // Link an AudioClip for Salsa3D to play
	public var broadcastReciever : GameObject; // Link a GameObject that contains scripts to catch SALSA and RandomEyes events
	public var loadSalsaAndRandomEyes : boolean = false; // Click this in the inspector to add, configure, and remove a Salsa3D and RandomEyes3D component at runtime

	private var salsa3D : Salsa3D;
	private var re3D : RandomEyes3D;
	...
	function Update () {
		...
		if (loadSalsaAndRandomEyes && !salsa3D && !re3D) {
			...
			salsa3D.saySmallTrigger = 0.002; // Set the saySmall amplitude trigger
			salsa3D.sayMediumTrigger = 0.004; // Set the sayMedium amplitude trigger
			salsa3D.sayLargeTrigger = 0.006; // Set the sayLarge amplitude trigger
			salsa3D.audioUpdateDelay = 0.08; // Set the amplitutde sample update delay
			salsa3D.blendSpeed = 10; // Set the blend speed
			salsa3D.rangeOfMotion = 100; // Set the range of motion
			...
			re3D.rangeOfMotion = 100; // Set the eyes range of motion
			re3D.blendSpeed = 10; // Set the eyes blend speed
			re3D.blinkDuration = 0.05; // Set the blink duration
			re3D.blinkSpeed = 20; // Set the blink speed
			re3D.SetOpenMax(0); // Set the max eye open position, 0=max
			re3D.SetCloseMax(100); // Set the max eye close position, 100=max
